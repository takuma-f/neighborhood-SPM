...
editforms = (Text('username', u'ユーザー名', validators=(NotEmpty(), RegexValidator(r'[A-Za-z¥d]')),), Text('password', u'パスワード', validators=(NotEmpty(), RegexValidator(r'[A-Za-z¥d]')),), Submit('submit', u'ログイン'))
...
fixeduser = 'user'  # テストユーザー
fixedpass = 'pass'  # テストパス


@expose
def login(_request, username='', password=''):
    res = Response()
    values, errors = loginform.validate({'username': username, 'password': password})
    if errors or fixeduser != username or fixedpass != password:
        return loginform(_request, values, errors)
